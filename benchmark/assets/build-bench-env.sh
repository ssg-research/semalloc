...
if test "$setup_rocksdb" = "1"; then
  ...
  DISABLE_WARNING_AS_ERROR=1 DISABLE_JEMALLOC=1 CXX="wllvm++" CXXFLAGS="-Xclang -no-opaque-pointers -Xclang -mno-constructor-aliases" make db_bench -j $procs
...
if test "$setup_redis" = "1"; then
  ...
  USE_JEMALLOC=no MALLOC=libc BUILD_TLS=no make LLVM_COMPILER=clang LLVM_COMPILER_PATH=/app/llvm15/build/bin/ CC=wllvm CFLAGS="-Xclang -no-opaque-pointers -Xclang -mno-constructor-aliases -DNO_MALLOC_USABLE_SIZE" CXX="wllvm++" CXXFLAGS="-Xclang -no-opaque-pointers -Xclang -mno-constructor-aliases -DNO_MALLOC_USABLE_SIZE" -j $procs
  popd
fi
...
if test "$setup_bench" = "1"; then
  ...
  if test -f sh8bench-new.c; then
    ...
    dos2unix SH8BENCH.C
    patch -p1 -R -o sh8bench-new.c SH8BENCH.C sh8bench.patch
  fi
  popd
