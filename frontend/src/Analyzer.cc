...
cl::opt<string> OutputFileName(
        "output", cl::desc("<output file>"),
        cl::init("/app/semalloc/test/input/output.bc"));
...
int main(int argc, char **argv) {
    ...
    // preMLTA
    auto PrePass = PreMLTAPass(GlobalCtx.Modules.at(0).first);
    PrePass.run();
    ...
    ConstructCGPass CCGPass(&GlobalCtx);
    CCGPass.run();
    auto *FunctionMap = CCGPass.getFunctionMap();

    // Find Library Indirect Calls
    auto ReducePass = ReduceLibraryIndirectCall(GlobalCtx.Modules.at(0).first);
    ReducePass.run(FunctionMap);

    // postMLTA
    auto PostPass = PostMLTAPass(GlobalCtx.Modules.at(0).first, FunctionMap, PrePass.getInvokeMap());
    PostPass.run();

    InstrumentIRPass IIRPass(GlobalCtx.Modules.at(0).first, FunctionMap);
    IIRPass.run();
    // CCGPass.PrintConstructedCG();

	// Print final results
//	PrintResults(&GlobalCtx);

    // output the modified modules
    for (auto M: GlobalCtx.Modules) {
//        outs() << M.second << "\n";
//        M.first->print(outs(), nullptr);

        int bc_fd;
        sys::fs::openFileForWrite(OutputFileName, bc_fd);

        raw_fd_ostream bc_file(bc_fd, true, true);
        M.first->print(bc_file, nullptr);
    }
