...
bool MLTA::isCompositeType(llvm::Type *Ty) {
	if (Ty->isStructTy() 
			|| Ty->isArrayTy() 
			|| Ty->isVectorTy())
		return true;
...
Type *MLTA::getFuncPtrType(llvm::Value *V) {
	Type *Ty = V->getType();
...
bool MLTA::typeConfineInInitializer(GlobalVariable *GV) {
	...
	std::list<pair<Type *, int>>NestedInit;
...
bool MLTA::nextLayerBaseType(Value *V, std::list<typeidx_t> &TyList,
	...
	// Phi and Select 
		...
		for (unsigned i = 0, e = PN->getNumIncomingValues(); i != e; ++i) {
			Value *IV = PN->getIncomingValue(i);
			NextV = IV;
			ret = nextLayerBaseType(IV, TyList, NextV, Visited);
            if (ret) {
                globalRet = true;
            }
		}
